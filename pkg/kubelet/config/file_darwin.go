...
func (s *sourceFile) doWatch(logger klog.Logger) error {
	...
	if err != nil {
		return fmt.Errorf("unable to create fsnotify watcher: %v", err)
	}
	defer w.Close()

	err = w.Add(s.path)
	if err != nil {
		return fmt.Errorf("unable to create fsnotify watcher for path %q: %v", s.path, err)
	}
	...
	for {
		...
		case event := <-w.Events:
			if err = s.produceWatchEvent(logger, &event); err != nil {
				return fmt.Errorf("error while processing fsnotify event (%+v): %v", event, err)
			}
		case err = <-w.Errors:
